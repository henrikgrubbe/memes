...
import {AllProvidersExhaustedError, ProviderError, QuotaExhaustedError, RateLimitError} from "./errors.js";
...
const failingProvider = (
    error: ProviderError | RateLimitError | QuotaExhaustedError,
): ProviderFn => () => Effect.fail(error);

const generateFailure = (layer: Layer.Layer<ProvidersServiceTag>) =>
    Effect.runPromise(generateImage("make a meme").pipe(
        Effect.flip,
...
describe("provider error history", () => {
    it("reconstructs ProviderError with the failed attempt", async () => {
        const error = await generateFailure(makeProvidersLayer({
            [PRIMARY]: failingProvider(new ProviderError({provider: PRIMARY, detail: "unavailable"})),
        }));
    ...
    it("preserves history already attached to an error", async () => {
        ...
        const error = await generateFailure(makeProvidersLayer({
            [PRIMARY]: failingProvider(new ProviderError({provider: PRIMARY, detail: "unavailable", history: attached})),
        }));
    ...
    it("reconstructs QuotaExhaustedError before reporting all primaries exhausted", async () => {
        const error = await generateFailure(makeProvidersLayer({
            [PRIMARY]: failingProvider(new QuotaExhaustedError({provider: PRIMARY, detail: "no credits"})),
        }));

        expect(error).toBeInstanceOf(AllProvidersExhaustedError);
        expect(error.history).toEqual([
            {provider: PRIMARY, status: "failed", message: `${PRIMARY} is out of credits/quota: no credits`},
        ]);
    });
    ...
    it("keeps skipped-primary history before the terminal attempt", async () => {
        const error = await generateFailure(makeProvidersLayer({
            [PRIMARY]: failingProvider(new QuotaExhaustedError({provider: PRIMARY, detail: "no credits"})),
            [SECONDARY]: failingProvider(new ProviderError({provider: SECONDARY, detail: "unavailable"})),
        }));
