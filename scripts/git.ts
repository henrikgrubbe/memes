...
const exec = (executor: CommandExecutor.CommandExecutor, cmd: string): Effect.Effect<string, PushFailedError> =>
            ...
            return {
                commitAndPush: (memeId: string): Effect.Effect<void, PushFailedError> =>
                    Effect.gen(function* () {
                        ...
                        const pushAttempt = run(`git pull --rebase origin main`).pipe(
                            Effect.flatMap(() => run(`git push origin HEAD`)),
